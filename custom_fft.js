/**
 * Custom Fast Fourier Transform module by Taichi Aritomo
 * adapted from Ryohei Tanaka's cv library for JS
 * https://github.com/wellflat/imageprocessing-labs/tree/master/cv/fft
 * 
 * Performs 1D-FFT/IFFT, 2D-FFT/IFFT (radix-2), filtering, and spectrum generation
 */
...
(function() {
    ...
    var core = {
        // initialize to define input size (must be radix-2)
		init: function(n) {
            if (n !== 0 && (n & (n - 1)) === 0) {
                _n = n;
                core._initArray();
 ...
 */
(function() {
    var SpectrumRenderer; // top-level namespace
    var _root = this; // reference to 'window' or 'global'

    if (typeof exports !== 'undefined') {
        SpectrumRenderer = exports; // for CommonJS
    } else {
        SpectrumRenderer = _root.SpectrumRenderer = {};
    }
    ...
    var core = {
        ...
        // returns an array of n^2 grayscale pixels to render the spectrum
        render: function(re, im, islog) {
			...
			// normalize against the max for visualization
			// remove for non-visualization purposes ...
//            imax = 1 / max;
//            for (var j = 0; j < n2; j++) {
...
//            }
    ...
    // aliases (public APIs)
    SpectrumRenderer.init = core.init;
    SpectrumRenderer.render = core.render;
}).call(this);
